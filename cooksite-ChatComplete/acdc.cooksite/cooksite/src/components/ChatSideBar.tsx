...
import { User, Home, ChevronLeft, ChevronRight } from "lucide-react";
...
const ChatSideBar = ({ chats, chatId, isPro }: Props) => {
  ...
  return (
    <div className={`w-full h-screen flex flex-col p-4 text-gray-200 bg-[#323232] ${collapsed ? 'w-16' : ''}`}>
      {/* Mini Navbar */}
      <div className="flex justify-between mb-2">
        ...
        <button onClick={() => setCollapsed(!collapsed)}>
          {collapsed ? <ChevronRight className="w-6 h-6 text-white cursor-pointer" /> : <ChevronLeft className="w-6 h-6 text-white cursor-pointer" />}
        </button>
      </div>
      {!collapsed && (
        <>
